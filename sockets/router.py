...
class SocketRouter:
    ...
    Usage:
        ...
        class MyPage(ReactView):
            # pass an extended RoutedSocketHandler to use a Handler with more features than the base one
            socket = SocketRouter(handler=RoutedSocketHandler)

            def props(self):
                # props given to the javascript on first pageload via window.props
                return {'initial_data': {...}}
            ...
            def default_route(self, content=None):
                self.send_action('ERROR', {'details': 'Got unknown message: ' + str(content)})
    ...
    def route(self, pattern=None):
        ...
        """
        def wrapper(handler):
            self.Handler.routes.append((pattern, handler))  # if any duplicate routes, last defined wins
            return handler
        return wrapper
